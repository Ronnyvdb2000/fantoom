...
- Volledige backtest functionaliteit
"""
...
FALLBACK_TICKERS = {
    "048 Nasdaq/NYSE": ["AAPL","MSFT","NVDA","META","GOOGL","AMZN","TSLA","AMD","INTC","NFLX"],
    "041 Benelux": ["ASML","AD.AS","INGA.AS","PHIA.AS","UNA.AS","ABN.AS","NN.AS","RAND.AS","WKL.AS","BESI.AS"],
}
...
def _wilder_smooth(series: pd.Series, period: int) -> pd.Series:
    ...
    for i in range(period, len(valid)):
        idx = valid.index[i]
        result[idx] = result[valid.index[i-1]] * (period-1)/period + valid.iloc[i]/period
    return result
...
class LivePortfolio:
        ...
        if os.path.exists(LIVE_POSITIONS_FILE):
            ...
            for _, r in dfp.iterrows():
                self.positions[r["ticker"]] = LivePosition(
                    ticker=r["ticker"], strategy=r["strategy"], entry_date=r["entry_date"],
                    entry_price=float(r["entry_price"]), size=int(r["size"]), cost=float(r["cost"]),
    ...
    def save_state(self, date: str, prices: Dict[str, float]):
        ensure_csv_header(LIVE_POSITIONS_FILE, ["ticker","strategy","entry_date","entry_price","size","cost","sl","tp","atr","days_open"])
        with open(LIVE_POSITIONS_FILE, "w", newline="", encoding="utf-8") as f:
            ...
            for p in self.positions.values():
                w.writerow([p.ticker, p.strategy, p.entry_date, p.entry_price, p.size, p.cost,
                            p.sl if p.sl else "", p.tp if p.tp else "", p.atr, p.days_open])
    ...
    def open_position(self, sig: Signal, prices: Dict[str, float]) -> Optional[LivePosition]:
        ...
        p = LivePosition(ticker=sig.ticker, strategy=sig.strategy, entry_date=sig.date.isoformat(),
                         entry_price=round(entry_prijs, 4), size=aandelen, cost=cost,
                         sl=round(stop, 4), tp=round(tp, 4), atr=sig.atr, days_open=0)
        self.positions[sig.ticker] = p
        self.log_trade(sig.date.isoformat(), sig.ticker, sig.strategy, "BUY",
                       entry_prijs, aandelen, cost, 0.0, 0.0, 0.0)
        return p
    ...
    def close_position(self, ticker: str, date: str, exit_price: float, reason: str):
        ...
        self.log_trade(date, ticker, p.strategy, "SELL", exit_slip, p.size, cost, pnl, tax, pnl - tax, reason)
        del self.positions[ticker]

    def log_trade(self, date, ticker, strategy, side, price, size, cost, pnl, tax, net, reason=""):
        ensure_csv_header(LIVE_TRADES_FILE, ["date","ticker","strategy","side","price","size","cost","pnl","tax","net","reason"])
...
def generate_exit_signals(portfolio: LivePortfolio, df: pd.DataFrame, date: dt.date) -> List[Signal]:
    ...
    for t, p in list(portfolio.positions.items()):
        ...
        if math.isnan(close):
            continue
        reason = None
        if p.sl and close <= p.sl:
            reason = f"Stoploss geraakt (SL={p.sl:.2f})"
        elif p.tp and close >= p.tp:
        ...
        if reason:
            signals.append(Signal(ticker=t, date=date, strategy=p.strategy, direction="SELL",
                                  reason=reason, price=close, atr=p.atr))
    return signals
...
class BacktestPortfolio:
    ...
    def close_position(self, ticker: str, date: dt.date, exit_price: float, reason: str):
        ...
        self.closed_trades.append({
            ...
            "ticker": ticker, "strategy": p.strategy, "entry_price": p.entry_price,
            "exit_price": round(exit_slip, 4), "size": p.size, "atr": p.atr,
...
def run_backtest():
    ...
    for date in all_dates:
        day_df = df[df["Date"] == pd.Timestamp(date)].copy()
        prices = {row["Ticker"]: safe_float(row["Close"]) for _, row in day_df.iterrows() if not math.isnan(safe_float(row.get("Close")))}
...
def format_signals_per_exchange(exchange_name: str, buy_signals: List[Signal], sell_signals: List[Signal],
                                portfolio: LivePortfolio, portfolio_waarde: float) -> Tuple[str, str]:
    nu = today_str()
    def koop_blok(signals):
        if not signals:
            return "_Geen signalen_"
        lines = []
        for s in signals:
            cfg = STRAT_CONFIG.get(s.strategy, {"sl_mult": 2.0, "tp_mult": 3.0})
            lines.append(f"• `{s.ticker}` {_strat_emoji(s.strategy)} *{s.strategy}* | EUR{s.price:.2f} | R/R:{s.rr_ratio:.1f} | [Grafiek]({_yahoo_link(s.ticker)})\n" +
                         sizing_tekst(s.ticker, s.price, s.atr, portfolio_waarde, cfg["sl_mult"], cfg["tp_mult"]))
        return "\n\n".join(lines)
    def verkoop_blok(signals):
        if not signals:
            return "_Geen signalen_"
        lines = []
        for s in signals:
            ...
            lines.append(f"• `{s.ticker}` {_strat_emoji(s.strategy)} *VERKOOP*\n  Reden : {s.reason}\n  Prijs : EUR{s.price:.2f}{pnl_str}\n  Commando: `/sell {s.ticker} {s.price:.2f} {size}`")
        return "\n\n".join(lines)
    deel1 = "\n\n".join([f"📊 *{exchange_name}*", f"_{nu} | Portfolio: EUR{portfolio_waarde:,.0f} | Risico: 5%/trade_",
    ...
    deel2_lines = [f"📊 *{exchange_name} (2/2)*", "", "🚀 *HYPER TREND:*", koop_blok([s for s in buy_signals if s.strategy == "Hyper Trend"]),
    ...
    for t, pos in portfolio.positions.items():
        deel2_lines.append(f"  • `{t}`: {pos.size}x @ EUR{pos.entry_price:.2f} ({pos.strategy}, {pos.days_open}d | SL:{format_price(pos.sl)} TP:{format_price(pos.tp)})")
    deel2_lines += ["", "⚙️ *PARAMETERS:*", f"_ATR-sizing: 5% risico/trade | Slippage: 0.1%_",
                    f"_Entry: open T+1 | SL: 2×ATR | TP: 3-5×ATR_", f"_ADX>15 filter | Wilder smoothing | Time-exit: {MAX_HOLD_DAYS}d_"]
    return deel1, "\n".join(deel2_lines)
